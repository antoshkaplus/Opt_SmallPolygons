...
//                                   const grid::Grid<City>& closest_cities, 
//                                   const unordered_set<uint64_t>& excluded_starting_tours,
//                                   Count max_tour_count, Seed seed, double prob_to_take_2) {
//    std::default_random_engine rng(seed);
//    Count city_count = closest_cities.row_count();
//    vector<City> cities(city_count);
//    iota(cities.begin(), cities.end(), 0);
...
//                if (tr.IsInside(ps[k_1])) continue;
//                k_1 = k_3;
//            } else { // b_2 visited
//                PointInsideTriangle<i::Point> tr(ps[k_0], ps[k_3], ps[k_1]);
//                if (tr.IsInside(ps[k_2])) continue;
//                k_2 = k_3;
//            }
//        }
//        // not a fact that k_i is inside cities
...
//        Index min_t = city_groups[min_e.first];
//        // start update adj_edges
//        auto& fst = adj_edgs[c_p.first];
//        if (fst.first == c) fst.first = c_p.second;
//        else {
//            fst.second = c_p.second;
//        }
//        
//        auto& snd = adj_edgs[c_p.second];
//        if (snd.first == c) snd.first = c_p.first;
//        else {
//            fst.second = c_p.first;
//        }
//        c_p = min_e;
//        // finish update adj_edges
//        // now update groups and tours
//        city_groups[c] = min_t;
//        remove(tours[c_t].begin(), tours[c_t].end(), c);
//        tours[c_t].pop_back();
//        InsertCityBetween(tours[min_t], c, {min_e.first, min_e.second});
//    }
//}
//
//
...
//                sol_tours = simplex_insertion->Solve(points_int, closest_cities, edge_distance, starting_tours);
//                ++iter;
...
//                    if (!MakeTourFeasible(points_int, closest_cities, edge_valid, adj_edges, excluded_starting_tours, t, 10)) {
//                        solo_success = false;
//                        break;
//                    } 
//                }
...
//                    bool resolve = ResolveIntersectionsBetweenTwoTours(points_int, closest_cities, 
//                                                                       sol_tours, adj_edges, excluded_starting_tours,
//                                                                       10);
//                    if (resolve) {
//                        // check intersections everywhere
//                        break;
//                    }
//                }
//            }
