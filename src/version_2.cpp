...
void SwitchEdgesInTour(Tour& tour, Edge& e_0, Edge& e_1);
...
vector<Tour> GenerateStartingTours(const grid::Grid<City>& closest_cities, 
    ...
    for (auto t = 0; t < max_tour_count; ++t) {
        ...
        City k_0 = cities[distr(rng)];
        remove(cities.begin(), cities.end(), k_0);
        cities.pop_back();
        ...
        remove(cities.begin(), cities.end(), k_1);
        cities.pop_back();
        remove(cities.begin(), cities.end(), k_2);
        cities.pop_back();
...
void SwitchEdgesInTour(Tour& tour, Edge& e_0, Edge& e_1) {
    ...
    // need to find k
    int k = max(max(oe_0[0], oe_0[1]), max(oe_1[0], oe_1[1]));
    (oe_0[0] += k) %= n;
    (oe_0[1] += k) %= n;
    (oe_1[0] += k) %= n;
    (oe_1[1] += k) %= n;
    ...
    // 0, 1, 2, 3
    // reverse is possible for 1 -> 2
    // also 3 -> 0
    // need to find previous order
    int d_1 = (oe_0[1] + n - k) % n;
    int d_2 = (oe_1[0] + n - k) % n;
    City *c_1 = &e_0[1];
    City *c_2 = &e_1[0];
    if (d_1 > d_2) {
        // can't reverse curent way
        d_1 = (oe_1[1] + n - k) % n;
        d_2 = (oe_0[0] + n - k) % n;
        c_1 = &e_0[1];
        c_2 = &e_1[0];
    ...
    swap(*c_1, *c_2); 
    reverse(tour.begin() + d_1, tour.begin() + d_2 + 1);
} 
...
bool MakeTourFeasible(const vector<i::Point>& points,
    ...
    while (true) {
        ...
        auto adj_edges = ConstructAdjacentEdges({tour}, city_count);
        ...
        Tour new_tour;
        for (auto i : inters) {
            if (!Segment{points[i.first[0]], points[i.first[1]]}.Intersect(
                                                                           Segment{points[i.second[0]], points[i.second[1]]})) {
                continue;
            }
            new_tour = tour;
            SwitchEdgesInTour(new_tour, i.first, i.second);
            auto c_0 = i.first[0];
            auto k_0 = i.first[1];
            auto c_1 = i.second[0];
            auto k_1 = i.second[1];
...
//
//bool ResolveIntersectionBetweenTwoTours(Tour& t_0, Edge& e_0_0, Edge& e_0_1, 
//                                        Tour& t_1, Edge& e_1_0, Edge& e_1_1) {
//    
//
//}
//
...
public:
    ...
    vector<string> choosePolygons(vector<int> point_coordinates, int max_tour_count) {
        ...
        edge_groups.resize(city_count, city_count);
        ...
        while (true) {
            ...
            for (auto& t : sol_tours) {
                ...
                if (!inters.empty()) {
                    if (!MakeTourFeasible(points_int, closest_cities, edge_valid, t)) {
                        solo_success = false;
                        break;
                    } 
                }
        ...
        cout << "area: " << ::ComputeArea(points_int, sol_tours) << endl << endl;
        vector<string> sol;
        for (auto& t : sol_tours) { 
            string result = "";
            for (int k : t) {
